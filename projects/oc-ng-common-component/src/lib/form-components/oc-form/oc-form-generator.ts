...
export class OcFormGenerator {
        ...
        case 'dynamicFieldArray':
          ...
          if (inputTemplate.defaultValue && inputTemplate.defaultValue.length > 0) {
            inputTemplate.defaultValue.forEach(dValue => {
              const form = [...inputTemplate.fields];
              form.forEach(field => {
   ...
   */
  static setValidators(control: AbstractControl, attributes,
   ...
   */
  static validatorMinLengthArray(min: number, showLengthErrorText?: boolean) {
   ...
   */
  static validatorMaxLengthArray(max: number, showLengthErrorText?: boolean) {
    return (c: AbstractControl): { [key: string]: any } => {
